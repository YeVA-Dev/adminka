!function($) {
	...
	baron.init = function(root, gData) {
		var viewPortHeight, topHeights, rTimer, selector, event, dom, scroller, container, bar, barTop, drag, scrollerY0;
		...
		function selection(on) {
			...
			// dom(document.body).css('MozUserSelect', on ? '' : 'none' ); //
		...
		event(bar, 'mousedown', function(e) {
			e.preventDefault(); // Text selection disabling in Opera... and all
			// other browsers?
		...
		event(document, 'mousedown', function(e) { // document, not window, for
			// ie8
			scrollerY0 = e.clientY - barTop;
		});

		event(document, 'mousemove', function(e) { // document, not window, for
			// ie8
			if (drag) {
